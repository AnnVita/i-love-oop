...
BOOST_FIXTURE_TEST_SUITE(Car, CarFixture)
	...
	BOOST_FIXTURE_TEST_SUITE(when_engine_turned_on, when_engine_turned_on_)
		BOOST_AUTO_TEST_CASE(car_speed_is_zero_immediately_after_turning_on)
		{
			BOOST_CHECK(car.GetSpeed() == 0);
		}

		BOOST_AUTO_TEST_CASE(car_direction_is_NONE_immediately_after_turning_on)
		{
			BOOST_CHECK(car.GetDirection() == Direction::NONE);
		}

		BOOST_AUTO_TEST_CASE(car_gear_is_NEUTRAL_immediately_after_turning_on)
		{
			BOOST_CHECK(car.GetGear() == Gear::NEUTRAL);
		}

		BOOST_AUTO_TEST_CASE(car_engine_can_be_turned_off_immediately_after_turning_on)
		{
			BOOST_CHECK(car.TurnOffEngine());
		}

		BOOST_AUTO_TEST_CASE(car_gear_can_be_changed_to_REVERSE_immediately_after_turning_on)
		{
			BOOST_CHECK(car.SetGear(Gear::REVERSE));
		}

		BOOST_AUTO_TEST_CASE(car_gear_can_be_changed_to_FIRST_immediately_after_turning_on)
		{
			BOOST_CHECK(car.SetGear(Gear::FIRST));
		}

		BOOST_AUTO_TEST_CASE(car_gear_cant_be_changed_for_more_than_FIRST_gear_value_immediately_after_turning_on)
		{
		...
		BOOST_AUTO_TEST_CASE(car_gear_can_be_changed_to_REVERSE_if_speed_is_zero_and_gear_is_NEUTRAL)
		{
			BOOST_CHECK(car.GetSpeed() == 0);
			BOOST_CHECK(car.SetGear(Gear::REVERSE));
		}

		BOOST_AUTO_TEST_CASE(car_gear_can_be_changed_to_FIRST_from_NEUTRAL)
		{
			BOOST_CHECK(car.SetGear(Gear::FIRST));
		}

		BOOST_AUTO_TEST_CASE(car_accelerate_if_gear_is_not_NEUTRAL)
		{
			BOOST_CHECK(car.SetGear(Gear::FIRST));
		...
		BOOST_AUTO_TEST_CASE(car_slow_down_if_gear_is_not_NEUTRAL)
		{
			BOOST_CHECK(car.SetGear(Gear::FIRST));
			BOOST_CHECK(car.SetSpeed(30));
			BOOST_CHECK(car.GetSpeed() == 30);
		...
		BOOST_AUTO_TEST_CASE(car_slow_down_if_gear_NEUTRAL)
		{
			BOOST_CHECK(car.SetGear(Gear::FIRST));
			BOOST_CHECK(car.SetSpeed(30));
			BOOST_CHECK(car.GetSpeed() == 30);
